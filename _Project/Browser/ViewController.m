...
// Метод скачивания объявлен здесь внутри интерфейса
...
// Реализация нашего метода скачивания для palera1n
- (void)downloadFileFromURL:(NSURL *)url {
...
// Полноценное восстановление всех оригинальных хост-методов
...
// Чистая объединенная функция (без дубликатов на выходе)
- (BOOL)webView:(id)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(NSInteger)navigationType {
    ...
    if ([downloadableTypes containsObject:extension]) {
        [self downloadFileFromURL:url];
        return NO; // Останавливаем веб-переход
    }
    
    // 2. Стандартный функционал оригинального браузера
